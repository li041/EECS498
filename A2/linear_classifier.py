...
def svm_loss_naive(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # Right now the dW is a sum over all training examples, but we want it to be an average instead
    dW /= num_train
...
def svm_loss_vectorized(
    ...
    S = torch.mm(X, W)
    row_idx = torch.arange(S.shape[0])
    s_yi = S[row_idx, y].view(num_train, -1)
    margin = (S - s_yi) + 1 
    # 方便算梯度时, 不用格外考虑y_i = j
    # loss可以不做这步处理, 而是在最后loss - num_tarin即可
    margin[row_idx, y] -= 1
    margin[margin < 0] = 0
    loss = torch.sum(margin) 
    loss /= num_train
    # add L2 Regularization, remember reg as regulation strength
    loss += reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    # Replace "pass" statement with your code
    margin[margin > 0] = 1
    # now margin is a matrix of 01
    margin[row_idx, y] = -torch.sum(margin, dim=1)
    dW = torch.mm(X.t(), margin) 
    # average
    dW /= num_train
    # add L2 Regularization grad 
    dW += 2 * reg * W
    #############################################################################
    #                              END OF YOUR CODE                              #
    #############################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    _, y_pred = torch.max(torch.mm(X, W), dim = 1)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 5e-2, 1e-2]
    regularization_strengths = [1e-3, 5e-4, 1e-4]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    # loss_history = cls.train(X_train, y_train, lr, reg, num_iters, batch_size=None, verbose=False)
    _ = cls.train(X_train, y_train, lr, reg, num_iters, batch_size=200, verbose=False)
    y_train_pred = cls.predict(X_train)
    y_val_pred = cls.predict(X_val)
    train_acc = torch.sum(y_train == y_train_pred) / y_train.shape[0]
    val_acc = torch.sum(y_val == y_val_pred) / y_val.shape[0]
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        # avoid numeric instability: overflows and underflows
        scores = scores - torch.max(scores)
        exp_s = torch.exp(scores)
        normalized_exp_s = exp_s / torch.sum(exp_s)
        loss += -torch.log(normalized_exp_s[y[i]]).item()
        # y_i != j, dW[:, j] += X[i] * normalized_exp_s[j]
        # y_i = j, dw[:, y[i]] += X[i] * (normalized_exp_s[y[i]] - 1)
        # broadcast 
        dW += torch.outer(X[i], normalized_exp_s)
        dW[:, y[i]] -= X[i]
    # Right now the loss is a sum over all training examples, but we want it
    # to be an average instead so we divide by num_train.
    loss /= num_train
...
def softmax_loss_vectorized(
    ...
    num_train = X.shape[0]
    loss = 0.0
    row_idx = torch.arange(num_train)
    scores = torch.mm(X, W)
    # avoid numeric instability: overflows and underflows
    scores = scores - torch.max(scores, dim = 1, keepdim = True)[0]
    exp_s = torch.exp(scores)
    # broadcast
    normalized_exp_s = exp_s / torch.sum(exp_s, dim=1, keepdim=True)
    loss = -torch.sum(torch.log(normalized_exp_s[row_idx, y]))
    normalized_exp_s[row_idx, y] -= 1
    dW = torch.mm(X.t(), normalized_exp_s)
    # average
    loss /= num_train
    dW /= num_train
    # L2 Regularization
