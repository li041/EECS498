...
def nn_forward_backward(
    ...
    loss = -torch.sum(torch.log(normalized_exp_s[range(N), y]))
    # average 
    loss /= N 
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    normalized_exp_s[range(N), y] -= 1 
    grads['W2'] = torch.mm(h1.t(), normalized_exp_s) / N + 2 * reg * W2 
    # y_i != j, grads_bi = p_j(p_j是normalized后的第i个data的标签j概率分布)
    # y_i = j, grads_bi = p_j - 1
    grads['b2'] = torch.sum(normalized_exp_s, dim = 0) / N
    h1[h1 > 0] = 1
    grads['b1'] = torch.sum(torch.mm(normalized_exp_s, W2.t()) * h1, dim = 0) / N 
    grads['W1'] = torch.mm(X.t(), torch.mm(normalized_exp_s, W2.t()) * h1) / N + 2 * reg * W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.7, 0.6, 0.5]
    hidden_sizes = [1024]
    regularization_strengths = [1e-5, 3e-5, 7e-5]
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    lrs, hs, reg, lrds = get_param_set_fn()
    pram_dict = {}
    pram_dict['lr'] = []
    pram_dict['h'] = []
    pram_dict['r'] = []
    ...
    for lr in lrs:
        for h in hs:
            for r in reg:
                for lrd in lrds:
                    net = TwoLayerNet(data_dict['X_train'].shape[1], h, 10)
                    stat = net.train(data_dict['X_train'], data_dict['y_train'],
                                     data_dict['X_val'], data_dict['y_val'],
                                     learning_rate=lr, reg=r, num_iters=3000,
                                     batch_size=1000, learning_rate_decay=lrd)
                    val_acc = stat['val_acc_history'][-1]
                    if val_acc > best_val_acc:
                        best_net = net
                        best_stat = stat
                        best_val_acc = val_acc
                        pram_dict['lr'].append(lr)
                        pram_dict['h'].append(h)
                        pram_dict['r'].append(r)
                        pram_dict['lrd'].append(lrd)
    print("parameters: ", pram_dict)
    print(f"best parameters: lr:{pram_dict['lr'].pop()}, h:{pram_dict['h'].pop()} r:{pram_dict['r'].pop()}, lrd:{pram_dict['lrd'].pop()}")
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
