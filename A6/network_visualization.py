...
def compute_saliency_maps(X, y, model):
    ...
    loss.backward()
    # Get the saliency map(absolute value of the gradient)
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        ...
        # L2 norm 
        dX = learning_rate * grad / grad.norm(p=2)
        # Update the adversarial image
        X_adv.data += dX * learning_rate
        # Optionally, clamp the values to stay within valid image range
        ...
        with torch.no_grad():
            ...
            if torch.argmax(model(X_adv)) == target_y:
                break
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    ########################################################################
    # Replace "pass" statement with your code
    target_score = model(img)[0, target_y]
    loss = target_score - l2_reg * img.norm(p=2)
    loss.backward()
