...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.bmm(features.view(N, C, -1), features.view(N, C, -1).transpose(1, 2))
    if normalize:
      gram /= (H * W * C) 
    ############################################################################
    #                               END OF YOUR CODE                           #
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    style_loss = sum([style_weights[i] * torch.sum((gram_matrix(feats[style_layers[i]]) - style_targets[i]) ** 2) for i in range(len(style_layers))])
    return style_loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  F_masked = features * masks.view(N, R, 1, H, W)
  guided_gram = torch.bmm(F_masked.view(N * R, C, -1), F_masked.view(N * R, C, -1).transpose(1, 2)).view(N, R, C, C)
  if normalize:
     guided_gram /= (H * W * C)
  return guided_gram
  ##############################################################################
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    style_loss = sum([style_weights[i] * torch.sum((guided_gram_matrix(feats[style_layers[i]], content_masks[style_layers[i]]) - style_targets[i]) ** 2) \
