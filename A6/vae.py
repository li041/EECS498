...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        mu = self.mu_layer(hidden_layer)
        logvar = self.logvar_layer(hidden_layer)
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        hidden_layer = self.encoder(torch.cat((torch.flatten(x, start_dim=1), c), dim=1))
        mu = self.mu_layer(hidden_layer)
        logvar = self.logvar_layer(hidden_layer)
        z = reparametrize(mu, logvar)
...
def reparametrize(mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    # sample epsilon from a standard normal distribution N ~ (0, 1)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    # p(z) is a Z-dimensional standard Gaussian N ~ (0, 1)
    D_KL = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    # average
    loss = (reconstruction_loss + D_KL) / x.size(0)
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
