...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        # self.params['W1'] = torch.nn.init.normal_(torch.empty(input_dim, hidden_dim), mean=0, std=weight_scale)
        ...
        # self.params['W2'] = torch.nn.init.normal_(torch.empty(hidden_dim, num_classes), mean=0, std=weight_scale)
    ...
    def loss(self, X, y=None):
        ...
        scores, L_cache = Linear.forward(a, self.params['W2'], self.params['b2']) 
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dx, grads['W2'], grads['b2'] = Linear.backward(dsm, L_cache)
        grads['W2'] += 2 * self.reg * self.params['W2']
        _, grads['W1'], grads['b1'] =  Linear_ReLU.backward(dx, LR_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        caches = []
        num_layers = self.num_layers
        # L-1 Linear_ReLU without dropout at test time 
        if mode == 'test' or (not self.use_dropout):
            for i in range(1, num_layers):
                out, cache = Linear_ReLU.forward(out ,self.params[f'W{i}'], self.params[f'b{i}'])
                caches.append(cache)
        else:
            # with dropout: linear - RELU - dropout  
            for i in range(1, num_layers):
                out, LR_cache = Linear_ReLU.forward(out ,self.params[f'W{i}'], self.params[f'b{i}'])
                out, dt_cache = Dropout.forward(out, self.dropout_param)
                cache = (LR_cache, dt_cache)
                caches.append(cache)

        scores, cache = Linear.forward(out, self.params[f'W{num_layers}'], self.params[f'b{num_layers}'])
        caches.append(cache)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        loss += 0.5 * reg * (self.params[f'W{num_layers}'] ** 2).sum()
        dx, grads[f'W{num_layers}'], grads[f'b{num_layers}'] = Linear.backward(dx, caches.pop()) 
        grads[f'W{num_layers}'] += reg * self.params[f'W{num_layers}']

        # L-1 Linear_Relu without dropout
        if not self.use_dropout:
            for i in reversed(range(1, num_layers)):
                loss += 0.5 * reg * (self.params[f'W{i}'] ** 2).sum()
                dx, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dx, caches.pop()) 
                # add regularization(note L2 regularization includes a factor of 0.5)
                grads[f'W{i}'] += reg * self.params[f'W{i}']
        else:
            for i in reversed(range(1, num_layers)):
                loss += 0.5 * reg * (self.params[f'W{i}'] ** 2).sum()
                cache = caches.pop()
                dx = Dropout.backward(dx, cache[1])
                dx, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dx, cache[0]) 
                # add regularization(note L2 regularization includes a factor of 0.5)
                grads[f'W{i}'] += reg * self.params[f'W{i}']
...
def create_solver_instance(data_dict, dtype, device):
    ...
    # modife mode
    model.reg = 5e-5
    # optim_config: a dict of hyperparams contains lr
    optim_config = {'learning_rate' : 0.5}
    lr_decay = 0.95
    # batch_size = 1000
    num_epochs = 20
    solver = Solver(model, data_dict, optim_config=optim_config, lr_decay=lr_decay, device=device)
...
def rmsprop(w, dw, config=None):
    ...
    w += (-dw) * config['learning_rate'] / (torch.sqrt(config['cache']) + config['epsilon'])
    next_w = w
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            # Inverted dropout: rescale at train time 
            out = x * mask / (1 - p)
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
            ...
            ##############################################################
            # Replace "pass" statement with your code
            # In test mode, just return the input and the mask in None 
            mask = None
            out = x 
            ##############################################################
            #                      END OF YOUR CODE                      #
