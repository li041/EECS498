...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        # assume two conv layers in stem 
        stem_cls = [
            nn.Conv2d(in_channels, stem_channels[0], kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(stem_channels[0], stem_channels[1], kernel_size=3, padding=1),
            nn.ReLU(),
        ] 
        # initialize weights of each conv layer from a normal distribution with mean = 0 and std dev = 0.01
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        # get class logits, box regression deltas and centerness logits for each level
        # original output shape is (batch_size, C, H, W), we need to permute them to (batch_size, H * W, C)
        # and reshape them to (batch_size, H * W, num_classes), (batch_size, H * W, 4), (batch_size, H * W, 1)
        # class_logits = {
        #     level_name: self.pred_cls(self.stem_cls(feats_per_fpn_level[level_name])).permute(0, 2, 3, 1).reshape(feats_per_fpn_level[level_name].shape[0], -1, self.pred_cls.out_channels)
        #     for level_name in feats_per_fpn_level.keys()
        # }
        # use focal loss, so we don't need to apply sigmoid to class logits
        class_logits = {
            level_name: self.pred_cls(self.stem_cls(feats_per_fpn_level[level_name])).permute(0, 2, 3, 1).reshape(feats_per_fpn_level[level_name].shape[0], -1, self.pred_cls.out_channels)
            for level_name in feats_per_fpn_level.keys()
        }
        # centerness and box regression predictors share the same stem_box
        boxreg_deltas = {  
            level_name: self.pred_box(self.stem_box(feats_per_fpn_level[level_name])).permute(0, 2, 3, 1).reshape(feats_per_fpn_level[level_name].shape[0], -1, self.pred_box.out_channels)
            for level_name in feats_per_fpn_level.keys()
        }
        centerness_logits = {
            level_name: self.pred_ctr(self.stem_box(feats_per_fpn_level[level_name])).permute(0, 2, 3, 1).reshape(feats_per_fpn_level[level_name].shape[0], -1, self.pred_ctr.out_channels)
            for level_name in feats_per_fpn_level.keys()
        }
        ######################################################################
        #                           END OF YOUR CODE                         #
...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        # 0 ~ 64 for p3, 64 ~ 128 for p4, 128 ~ inf for p5
        ...
        # broadcast gt_areas to match_matrix shape here
        # 1e8是一个非常大的数，用来确保即使在减去gt_areas（真实框的面积）之后，匹配矩阵中的值仍然是正的。
        # 这种方法可以帮助模型在有多个匹配时，倾向于选择面积较小的真实框，因为较小的面积会导致较小的减值，从而在乘法操作后保留较大的匹配分数。
        ...
        # 在这行代码matched_idxs[match_quality < 1e-5] = -1中，1e-5用作一个阈值来判断锚点是否找到了与之匹配的真实框。
        # match_quality表示每个锚点与其匹配的真实框之间的匹配质量（或分数）
        # ，如果这个分数小于1e-5，则认为该锚点没有找到合适的匹配，因此将其索引设置为-1，表示未匹配。
        # 这个阈值的设置是为了避免由于计算精度问题导致的极低匹配分数被错误地认为是有效匹配 
...
def fcos_get_deltas_from_locations(
    ...
    deltas = None
    # Replace "pass" statement with your code
    # gt_boxes and locations是匹配的
    deltas = torch.zeros(gt_boxes.shape[0], 4, device=gt_boxes.device)
    deltas[:, 0:2] = locations - gt_boxes[:, 0:2]
    deltas[:, 2:4] = gt_boxes[:, 2:4] - locations
    deltas = deltas / stride
    # background boxes will be all -1s, and its delta should also be all -1s
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # for background boxes, the output_boxes should be its locations
    # eg. deltas = (-1, -1, -1, -1), locations = (5, 10), output_boxes = (5, 10, 5, 10)
    output_boxes = deltas.clone().clamp(min=0) * stride
    output_boxes[:, 0:2] *= -1
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        locations_per_fpn_level = get_fpn_location_coords(shape_per_fpn_level, strides_per_fpn_level, dtype=dtype, device=device) 
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for b in range(images.shape[0]):
            matched_gt_deltas.append({
                level_name: fcos_get_deltas_from_locations(locations, matched_gt_boxes[b][level_name], stride)
                for level_name, locations, stride in zip(locations_per_fpn_level.keys(), locations_per_fpn_level.values(), strides_per_fpn_level.values())
            })
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # Replace "pass" statement with your code
        # there are -1s in matched_gt_boxes, the corresponding one hot encoding should be all 0s
        # gt_classes = matched_gt_boxes[:, :, 4].clip(min=0)
        gt_classes = matched_gt_boxes[:, :, 4] + 1
        # print(gt_classes)
        # one hot encoding
        # for background boxes, one hot encoding should be all 0s
        gt_classes_one_hot = F.one_hot(gt_classes.to(torch.int64), self.num_classes+1).to(torch.float32)
        gt_classes_one_hot = gt_classes_one_hot[:, :, 1:]
        ...
        # No boxreg loss for background locations
        ...
        # the argument of fcos_make_centerness_targets is LTRB deltas: (N, 4) -> (N, )
        ...
        # No centerness loss for background locations
        loss_ctr[(matched_gt_deltas < 0).all(dim=1)] *= 0.0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # print(level_pred_scores, level_pred_classes)
            ...
            # Step 3:
            # Replace "pass" statement with your code
            # first, clip uninterest locations
            level_locations = level_locations[keep]
            # original level_deltas shape: (N, 4)
            level_deltas = level_deltas[keep, :]
            # level_pred_boxes shape: (N, 4)
            ...
            # Replace "pass" statement with your code
            # assume images shape: (B, C, H, W)
            height, width = images.shape[2], images.shape[3] 
            # x2 > x1, y2 > y1是由fcos_apply_deltas_to_locations保证的
            torch.clip_(level_pred_boxes[:, 0], min=0) 
            torch.clip_(level_pred_boxes[:, 1], min=0) 
            torch.clip_(level_pred_boxes[:, 2], max=width) 
            torch.clip_(level_pred_boxes[:, 3], max=height) 
