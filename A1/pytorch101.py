...
def create_tensor_of_pi(M: int, N: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # 1. x = torch.tensor([3.14] * N for _ in range(M))
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    # [start, stop]注意包含stop
    # list comprehension 1. x = torch.tensor([element for element in range(start, stop + 1) if element % 10 == 0], dtype=torch.float64)
    adjusted_start = (start + 9) // 10 * 10
    if adjusted_start > stop:
        return torch.tensor([], dtype=torch.float64)
    else: 
      x = torch.arange(adjusted_start, stop+1, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    #不用检查(M, N)
    x[:4, :6] = torch.tensor(([[0, 1, 2, 2, 2, 2] for _ in range(2)] + [[3, 4, 3, 4, 5, 5] for _ in range(2)]), dtype=x.dtype)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # 1. y = x.view(2, 3, -1).transpose(0, 1).contiguous().view(3, 8)
    y = x.view(2, 3, -1).transpose(0, 1).reshape(3, 8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    M = x.shape[0]
    mu_j = x.sum(dim=0) / M  
    # broadcast happens in x - mu_j and / (M - 1) 
    sigma_j = ( ((x - mu_j) ** 2).sum(dim=0) / (M - 1)) ** 0.5 
    y = (x - mu_j) / sigma_j
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    padded_tensor = torch.nn.utils.rnn.pad_sequence(xs, batch_first=True, padding_value=0)
    print(padded_tensor)
    y = padded_tensor.sum(dim=1) / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
