...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_vec = x_train.view(num_train, -1)    
    x_test_vec = x_test.view(num_test, -1)    
    for i in range(num_train):
      for j in range(num_test):
          dists[i][j] = torch.sqrt(torch.sum((x_train_vec[i] - x_test_vec[j]) ** 2)) 
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_vec = x_test.view(num_test, -1)
    for i in range(num_train):
      # broadcast happens in x_train[i] - x_test
      # dim=(1,2,3) which only keeps dim=0 which the result shape is (num_test, )
      dists[i] = torch.sqrt(torch.sum((x_train_vec[i] - x_test_vec) ** 2, dim=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_vec = x_train.view(num_train, -1)
    x_test_vec = x_test.view(num_test, -1)
    # Compute the squared Euclidean distance using PyTorch's broadcasting and matrix multiplication
    # x_train_sum_square的shape: (num_train, 1), x_test_sum_square的shape: (num_test, )
    x_train_sum_square = torch.sum((x_train_vec ** 2), dim=1, keepdim=True)
    x_test_sum_square = torch.sum((x_test_vec ** 2), dim=1, keepdim=True)
    dists = torch.sqrt(x_train_sum_square + x_test_sum_square.t() - 2 * torch.mm(x_train_vec, x_test_vec.t()))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, indices = torch.topk(dists, k=k, dim=0, largest=False)
    k_labels = y_train[indices]
    y_pred = torch.mode(k_labels, dim=0)[0]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        # 1. 准备train和validation
        # 选择第i个folk为validation, 剩下的torch.cat后为x_train
        x_validation_i = x_train_folds.pop(i)
        x_train_i = torch.cat(x_train_folds)
        y_validation_i = y_train_folds.pop(i)
        y_train_i = torch.cat(y_train_folds)
        # 2.predict 
        classifier_i = KnnClassifier(x_train_i, y_train_i)
        k_accuracy.append(classifier_i.check_accuracy(x_validation_i, y_validation_i, k=k, quiet=True))
        # 恢复x_train_folds和y_train_folds
        x_train_folds.insert(i, x_validation_i)
        y_train_folds.insert(i, y_validation_i)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accs in sorted(k_to_accuracies.items()):
      if best_mean < statistics.mean(accs):
        best_mean = statistics.mean(accs)
        best_k = k
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
