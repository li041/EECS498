...
def generate_token_dict(vocab):
    ...
    token_dict = {token:index for index, token in enumerate(vocab)}
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return token_dict
...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K, M = query.shape
    weight = torch.empty(K, K ,dtype=query.dtype, device=query.device)
    out = torch.empty_like(query)
    norm = M ** 0.5
    # weight: (K, K)
    for i in range(K):
        for j in range(K):
            weight[i][j] = torch.dot(query[i], key[j])
        # get all the K scaled weights corresponding to query[i, :]
        weight[i] = torch.softmax(weight[i] / norm, dim=0)
        out[i, :] = torch.matmul(weight[i], value)
    '''
    K, M = query.shape
    out = torch.zeros_like(query)

    for i in range(K):
        # Compute the scaled dot product of the i-th query with each key
        weights = torch.zeros(K)
        for j in range(K):
            weights[j] = torch.dot(query[i], key[j]) / torch.sqrt(torch.tensor(M, dtype=torch.float))
...
def scaled_dot_product_two_loop_batch(
    ...
    '''
    for i in range(N):
        weight = torch.empty(K, M, dtype=query.dtype, device=query.device)
        # weight: (K, K)
        for j in range(K):
            # weight[i,j] denote the weight for query i, key j
            weight = torch.softmax(torch.mm(query[i], key[j]) / norm, dim=1)
    '''
    # weight: (N, K, K)
    weight = torch.bmm(query, key.transpose(1,2))
    weight = torch.softmax(weight / norm, dim=2)
    for i in range(N):
        for j in range(K):
            out[i, j] = torch.matmul(weight[i, j], value[i]) 
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        weight = torch.bmm(query, key.transpose(1,2))
        weight[mask] = -1e9
        weight_softmax = torch.softmax(weight / norm, dim=2)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # Replace "pass" statement with your code
        pass
        # Linear weight initialize from a uniform distribution bounded by [-c, c]
        # c = sqrt(6/ (D_in + D_out))
        ...
        torch.nn.init.uniform_(self.q.weight, -c_q, c_q)
        torch.nn.init.uniform_(self.k.weight, -c_q, c_q)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        list = [SelfAttention(dim_in, dim_out, dim_out) for _ in range(num_heads)]
        self.heads = nn.ModuleList(list)
    ...
    def forward(
        ...
        for head in self.heads:
            tensors_to_concat.append(head(query, key, value, mask))
        cats = torch.cat(tensors_to_concat, dim=-1)
        y = self.linear(cats)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        y = self.scale * x_norm + self.shift
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        # - feedforward - out3 - layer_norm(out3 + out2) - dropout - out
        # query, key, value are the same as input x
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    mask = torch.triu(torch.ones(K, K), diagonal=1).bool().unsqueeze(0).repeat(N, 1, 1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        out3 = self.attention_cross(out2, enc_inp, enc_inp)
        out4 = self.norm2(self.dropout(out2 + out3))
        out5 = self.feed_forward(out4)
        y = self.norm3(self.dropout(out4 + out5))
        '''
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    #  for a sequence of length K, assign the nth element in the sequence a value of n/K, where n starts from 0.
    y = torch.tensor([i / K for i in range(K)]).unsqueeze(1).repeat(1, M).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
